...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无",@"灰度",@"颠倒",@"旋涡",@"马赛克",@"马赛克2",@"马赛克3", @"缩放",@"灵魂出窍",@"抖动",@"闪白",@"毛刺"];
...
- (void)filterInit {
    ...
    //设置图层frame
    layer.frame = CGRectMake(0, 150, self.view.frame.size.width, self.view.frame.size.width);
    //设置图层的scale
    ...
    //9.设置默认着色器
    [self setupShaderProgramWithName:@"Normal"]; // 一开始选用默认的着色器
...
//普通图片显示着色器程序/灰度滤镜着色器程序/颠倒滤镜着色器程序/旋涡滤镜着色器程序/马赛克滤镜着色器程序/六边形马赛克滤镜着色器程序/三角形马赛克滤镜着色器程序/缩放滤镜着色器程序/灵魂出窍滤镜着色器程序/抖动滤镜着色器程序/闪白滤镜着色器程序/毛刺滤镜着色器程序
    
    NSArray *arr = @[@"Normal", @"Gray", @"Reversal", @"Cirlce", @"Mosaic", @"HexagonMosaic", @"TriangularMosaic", @"Scale", @"SoulOut", @"Shake", @"ShineWhite", @"Glitch"];
    [self setupShaderProgramWithName:arr[index]];
    // 重新开始滤镜动画
    [self startFilerAnimation];
...
//    6.打开 positionSlot 属性 并且传递数据到positionSlot中去(顶点坐标)
